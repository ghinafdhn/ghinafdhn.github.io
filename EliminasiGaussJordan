...
// buat variabel global terlebih dahulu
int i, j, k;

// buat fungsi eliminasi gauss jordan
void gaussJordan(double arr[size][size], int ordo){
    ...
    // fungsi perulangan untuk seluruh matriks
    for(i = 0; i < ordo; i++){
        pembagi = arr[i][i];

        // fungsi perulangan untuk tiap-tiap baris pada matriks
        for(k = i; k < ordo + 1; k++){
            arr[i][k] /= pembagi;
        }

        // fungsi perulangan untuk tiap-tiap kolom pada matriks
        // cek secara vertikal agar array selain x[i][i] bernilai 0
        for(j = i + 1; j < ordo + 1; j++){
            v[i] = arr[j][i];

            // fungsi perulangan untuk tiap-tiap baris pada matriks kolom yang sama
            for(k = i; k < ordo + 1; k++){
...
// buat fungsi untuk mencetak matriksnya
void printMatrix(double arr[size][size], int ordo){
    for (i = 0; i < ordo; i++){
        ...
        for (j = 0; j < ordo; j++){
            ...
            // fungsi untuk menuliskan vektor matriks B
            if (j == ordo - 1){
...
// main fungsi
int main(){
    // deklarasi tipe data terlebih dahulu
    ...
    // fungsi untuk inputan ordo kurang dari 2
    if(ordo < 2){
    ...
    // fungsi untuk Deteksi Input User
    // fungsi Perulangan Tiap Baris Matriks
    for(i = 0; i < ordo; i++) {
